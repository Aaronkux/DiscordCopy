...
const SettingSVGWrapper = styled.div`
  line-height: 0;
...
const AddLinkSVGWrapper = styled.div`
  fill: #e6e6e6;
  line-height: 0;
  margin-right: 5px;
  cursor: pointer;
...
function arrangeChannels(channels: ChannelType[]) {
  const rootChannels: CombinationChannel[] = channels.filter(
...
function Channel() {
  ...
  const channels = guild?.channelsUids?.map(
    (channelId) => channelState[channelId.toString()]
  )
  const renderChannels = channels
    ? arrangeChannels(JSON.parse(JSON.stringify(channels)))
    : undefined
  ...
  return (
    <Container>
      <Header>
        ...
        {guildId ? (
          <SVGWrapper>
            <AddLinkSVGWrapper onClick={() => setVisible(true)}>
              <AddLinkSvg />
            </AddLinkSVGWrapper>
            <SettingSVGWrapper
              onClick={() => {
      ...
      >
        {renderChannels
          ? renderChannels.map((renderChannel) => {
              if (renderChannel.channelType === 0) {
                const renderChild = renderChannel.childChannels?.map(
                  (childChannel) => (
                    <Text
