      ...
      <Separator />
      {
        Object.values(guilds).map(guild =>
          <NavButton
            ...
            avatar={guild.avatar}
            changeGuildhandler={() => {dispatch(websocketFns.changeCurrentGuild(guild.uid))}}
          />)
      }
      <Separator />
      <NavButton name='新增一个工会' url='/add' isSvg handleClick={(e) => { e.preventDefault(); dispatch(topLayerFns.toggleNewOrJoinLayer()) }}>
        <PlusSvg />
      </NavButton>
    </SideBar >
  )
